...
pdf_pages[[i]] <- pdf_subset("R/input/RNT_202402.pdf", pages = pdf_range[i]:pdf_range[i+1])
...
for (i in seq_along(pdf_length)) {
    ...
    if (!is.na(delim1) & !is.na(delim2)) {
        ...
        row_to_add <- data.table(page = i, cuenta_cot = cuenta_cot, nif = nif)
...
pdf_subset("R/input/RNT_202402.pdf", pages = listed_pdfs[[i]], output = paste0("R/output/", unique_nifs[i], ".pdf"))
}
